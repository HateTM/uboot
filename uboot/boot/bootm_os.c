			       ...
			       struct bootm_headers *images)
{
	...
	data_kernel_load_addr = CONFIG_SYS_LOAD_ADDR;
#elif defined(CONFIG_LOADADDR)
	data_kernel_load_addr = CONFIG_LOADADDR;
#endif
	...
	flush_start = ALIGN_DOWN(data_uboot_load_addr, ARCH_DMA_MINALIGN);
	flush_size = ALIGN(SZ_8M, ARCH_DMA_MINALIGN); /* suppose uboot max to 8M*/

	flush_cache(flush_start, flush_size);

	flush_start = ALIGN_DOWN(data_kernel_load_addr, ARCH_DMA_MINALIGN);
	flush_size = ALIGN(SZ_64M, ARCH_DMA_MINALIGN); /* suppose kernel max to 64M*/
