...
static unsigned int rsa_read_int_from_public_key(char *bit)
{
	unsigned int re =0;
...
static void *rsa2_newkey_ex(char *data, int len)
{
	...
	bit_len = rsa_read_int_from_public_key((char*)&(rsa_pub_blob->rsa_pub_key.bitlen));
	
	rsa->exponent = RSA_bignum_from_int(rsa_read_int_from_public_key((char*)(&(rsa_pub_blob->rsa_pub_key.pubexp))));
...
static int rsaVerifySign(
	...
	unsigned int        public_key_blob_len,
	unsigned char *in_data,      
	unsigned int        in_data_len,		   
	unsigned char *in_Signature,   
	unsigned int        in_Signature_len)
{
	char signBuf[256];
 ...
 *			 												 unsigned int PublicKeyBlobLen,
 *			 												 unsigned char *pInData,      
 *			 												 unsigned int inDataLen,		   
 *			 												 unsigned char *PInSignature,   
 *			 												 unsigned int inSignatureLen)
