		       ...
		       bool fullerase)
{
	...
	if (offset + maxsize > mtd->size)
		maxsize = mtd->size - offset;

	limit = (start + maxsize + mtd->erasesize_mask) &
...
#if 0//defined(CONFIG_TP_FIRST_UBOOT)
...
static int read_ubi_volume_v2(const char *volume, void *buff, size_t size, bool no_copy)
{
...
static int boot_from_ubi(struct mtd_info *mtd)
{
	...
	data_uboot_load_addr = CONFIG_TEXT_BASE - 0x40; /* 0x41e00000 - 0x40 */
	...
	ret = read_ubi_volume_v2("uboot", (void *)data_uboot_load_addr, 0, false);
	if (ret)
	{
	...
	ret = read_ubi_volume_v2("rootfs", (void *)data_rootfs_load_addr, 0, true);
	if (ret)
	{
	...
	ret = read_ubi_volume_v2("kernel", (void *)data_kernel_load_addr, 0, false);
	if (ret)
	{
...
int is_recovery_button_pressed(void)
{
	...
	ret = ops->get_value(dev, GPIO_BUTTON_WPS);
